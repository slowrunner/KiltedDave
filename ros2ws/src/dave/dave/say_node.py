...
"""
    ...
    DOCKER INVOCATION REQ:
        docker run -dit --name r2hdp --net=host \
            ...
            -w /home/ubuntu/KiltedDave/ros2ws --privileged r2hdp 

    CONTROL VOLUME: (cmds/set_vol_very_low.sh)
        #!/bin/bash
        # FILE: cmds/set_vol_very_low.sh
        amixer -D pulse sset Master 10%
        ~/KiltedDave/ros2ws/cmds/say.sh 'Volume set very low 10 percent'
...
# voicedir = os.path.expanduser('~/KiltedDave/models/piper-tts/') #Where onnx model files are stored on my machine
voicedir = '/home/ubuntu/KiltedDave/models/piper-tts/'  # Where onnx model files are stored on my machine
...
class SayService(Node):
    ...
    def __init__(self):
        ...
        self.loghandler = logging.FileHandler('/home/ubuntu/KiltedDave/logs/speak.log')
