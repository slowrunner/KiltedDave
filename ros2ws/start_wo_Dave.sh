...
<< ////
    FILE:  /home/ubuntu/KiltedDave/ros2ws/start_wo_Dave.sh

    PURPOSE:  Start without dave_node for testing subsystems such as docking and battery

This file starts the needed KiltedDave nodes w/o starting the dave_node (7 python processes in status.sh):
...
x dave_node             calls /dock when vBatt<10v, and calls /undock when charge current < -175mA
